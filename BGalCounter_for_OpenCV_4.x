...
def identifyCells(img):
    ...
    contours, ____ = cv2.findContours(closed, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
...
def bGalFinder(file: chr, mask):
    ...
    contours, hierarchy = cv2.findContours(bDiffDilated, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
...
def parseArguments():
    ...
    requiredNamed.add_argument('-n', '--name', dest='name', default='BetaGalCounter', type=str,
